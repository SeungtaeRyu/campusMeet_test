...
class MyPageEditKeywordScreen extends StatefulWidget {
  ...
  const MyPageEditKeywordScreen({Key? key, required this.keywordsData}) : super(key: key);
...
class _MyPageEditKeywordScreenState extends State<MyPageEditKeywordScreen> {
  ...
  @override
  void initState() {
    for (int i = 0; i < widget.keywordsData.length; i++) {
      for (int j = 0; j < keywords.length; j++) {
        if (widget.keywordsData[i] == keywords[j]) {
          selects[j] = true;
          data.add(keywords[j]);
        }
      }
